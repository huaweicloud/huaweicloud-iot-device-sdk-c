...
static HW_INT FILE_HttpsWrite(SSL *ssl, HW_CHAR *url, HW_CHAR *filePath, HW_INT readOrwrite){
	...
	do{
		...
		if (res < 0) {
			PrintfLog(EN_LOG_LEVEL_ERROR, "FILE_datatrans: FILE_HttpsWrite() send https request failed, response is %d.\n", res);
			return FILE_FAILURE;
		}
...
HW_API_FUNC HW_INT FILE_OBSTransmission(HW_CHAR *url, HW_CHAR *filePath, HW_INT timeout, HW_INT readOrwrite){
	...
	if (url == NULL || filePath == NULL || timeout <= OTA_TIMEOUT_MIN_LENGTH) //the timeout value must be greater than 300s
	{
